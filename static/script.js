...
document.querySelectorAll('a[href^="#"]').forEach(anchor => {
  anchor.addEventListener('click', function(e){
    if (this.hasAttribute('data-software-modal')) return;
...
// Software lead modal
const softwareLeadModal = document.getElementById('softwareLeadModal');
if (softwareLeadModal) {
  const leadSource = document.getElementById('softwareLeadSource');
  const leadPageUrl = document.getElementById('softwareLeadPageUrl');
  const leadService = document.getElementById('softwareLeadService');
  const leadMessage = document.getElementById('softwareLeadMessage');
  const closeButtons = softwareLeadModal.querySelectorAll('[data-software-modal-close]');
  let previousFocus = null;

  function openSoftwareLeadModal(trigger) {
    previousFocus = document.activeElement;
    if (leadPageUrl) leadPageUrl.value = window.location.href;
    if (leadSource) leadSource.value = trigger.dataset.softwareSource || trigger.textContent.trim() || 'Software website';
    if (leadService && trigger.dataset.softwareInterest) leadService.value = trigger.dataset.softwareInterest;
    if (leadMessage && trigger.dataset.softwareMessage && !leadMessage.value) {
      leadMessage.value = trigger.dataset.softwareMessage;
    }
    softwareLeadModal.hidden = false;
    softwareLeadModal.setAttribute('aria-hidden', 'false');
    document.body.classList.add('software-modal-open');
    const firstField = softwareLeadModal.querySelector('form input:not([type="hidden"]), form select, form textarea');
    if (firstField) firstField.focus();
  }

  function closeSoftwareLeadModal() {
    softwareLeadModal.hidden = true;
    softwareLeadModal.setAttribute('aria-hidden', 'true');
    document.body.classList.remove('software-modal-open');
    if (previousFocus && typeof previousFocus.focus === 'function') previousFocus.focus();
  }

  document.querySelectorAll('[data-software-modal]').forEach(trigger => {
    trigger.addEventListener('click', event => {
      event.preventDefault();
      openSoftwareLeadModal(trigger);
    });
  });
  closeButtons.forEach(button => button.addEventListener('click', closeSoftwareLeadModal));
  window.addEventListener('keydown', event => {
    if (event.key === 'Escape' && !softwareLeadModal.hidden) closeSoftwareLeadModal();
  });
}
...
if (contactForm) {
  ...
  const submitBtnDefault = submitBtn ? submitBtn.innerHTML : '';
  contactForm.addEventListener('submit', async (e)=>{
    e.preventDefault();
    const pageUrlField = contactForm.querySelector('[name="page_url"]');
    ...
    try{
      ...
      if (submitBtn) { submitBtn.disabled=false; submitBtn.innerHTML=submitBtnDefault || '<i class="ti ti-send"></i> Send Message'; }
