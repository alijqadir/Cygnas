...
contactForm.addEventListener('submit', async (e)=>{
  e.preventDefault();
  submitBtn.disabled=true; submitBtn.innerHTML='<i class="ti ti-loader-2"></i> Sending...';
  ...
  try{
    ...
    submitBtn.disabled=false; submitBtn.innerHTML='<i class="ti ti-send"></i> Send Message';
...
// Particles (small sparkles) — optional, kept subtle
(function createParticles() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    p.style.cssText =
      "position:absolute;width:4px;height:4px;border-radius:50%;background:var(--accent-glow);opacity:.5;left:" +
      Math.random() * 100 +
      "%;animation:float-up " +
      (15 + Math.random() * 10) +
...
(function () {
  ...
  cards.forEach((c) => {
    ...
    c.style.transition = "transform .5s ease, opacity .5s ease";
  });
